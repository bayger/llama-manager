...
import { getVersionsDir, ConfigData } from "./config";
...
const reFileType = /print_info:\s*file type\s+=\s+(\S+)/;
const reFileSize = /print_info:\s*file size\s+=\s+([\d.]+\s*\w+)/;
const reGGUFVersion = /version GGUF V(\d+)/;
const reTensorType = /-\s+type\s+(\S+):\s+(\d+)\s+tensors?/;
